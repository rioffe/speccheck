...
> - **Status:** proposal, 2026-09-20; for `spec-writing` to turn into `SPEC.md` v1.15 rows after the requester settles D-26 and D-27 below (D-26 is numbered past `PROPOSAL_v1.14_obligation_aware_judge.md`'s own D-26, since that proposal is still pending too; renumber whichever lands second).
> - **Applies to:** `SPEC.md` v1.13 — C-01 (citation rule), C-03 (`Citation`/`TestCase`), C-06 (judge request), C-07 (JSON), C-10 (judge instruction text), §9's own preamble sentence, T-46/T-76 (golden fixture). Independent of `PROPOSAL_v1.14_obligation_aware_judge.md`: that proposal tells the judge about *neighboring* obligations a statement names (C-12 edges); this one tells it about a citation that names *no* neighbor at all, just a token reused as example data. Neither needs the other; both could land.
...
`K-15`'s clause-grounding check — the fix `PROPOSAL_v1.9_clause_grounding.md` built for exactly
