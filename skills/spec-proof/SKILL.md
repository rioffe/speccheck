---
name: spec-proof
description: Create the formal (Lean 4) half of the conformance evidence for a built system — a self-contained lake project in the project's `proof/` directory (`lean-toolchain`, `lakefile.toml`, `lake-manifest.json`, `README.md`) that transcribes the spec's deterministically-provable contract into a Lean model and proves it kernel-checked for all inputs, while mapping every unprovable row to the empirical test that carries it. Phases: read the spec and classify every ID as proven / structural / deferred (the scope map), scaffold the well-formed project, write the three files (Spec / Model / Theorems) one build per file, then audit (`lake build` 0 warnings, every ID tagged exactly once, statement-by-statement re-read of the spec) and commit. Use when asked to "prove the spec", "add a formal proof", or "build the Lean half" of a project that already has a SPEC.md and a green acceptance suite.
license: MIT
---
