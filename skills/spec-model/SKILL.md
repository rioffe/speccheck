---
name: spec-model
description: Build the formal (Lean 4) model of a SPEC.md when no implementation exists yet — a self-contained lake project in the project's `proof_from_spec/` directory (`lean-toolchain`, `lakefile.toml`, `lake-manifest.json`, `README.md`) that transcribes the spec's normative tables into a pure total Lean function, kernel-checks the spec's *derived* claims for all inputs (the exit/status map, the byte contracts, the diagnostics contract, the invariants, reachability, and coverage — that the spec's input space has no silent case), and reifies every under-specified or over-pinned case as a kernel-checked witness reported as an `F-nnn` finding in `docs/reviews/SPEC_MODEL_FINDINGS.md`. Phases are read the spec and classify every ID (proven / structural / deferred / excluded), write the scope map and the model sketch, scaffold the well-formed project, write the three files (Spec / Model / Theorems) one build per file, then audit (lake build 0 warnings, anti-tautology re-read, every ID tagged or tabled exactly once), write the findings, and commit. Use when asked to "prove the spec", "model the spec", "formalize SPEC.md", or to build the Lean half of a project that has a SPEC.md and no implementation yet.
license: MIT
---
