---
name: spec-proposal
description: Write a PROPOSAL document — a scoped, evidence-grounded change to an existing, already-implemented SPEC.md or tool, not a greenfield spec. Encodes the fixed six-section template (problem, change, cost, alternatives, decisions for the requester, what it doesn't fix) with its front-matter blockquote, draft R/C/I/K/E/T rows for spec-writing to fold in once confirmed, and the two-column decision table (ID | Statement, each cell an option-versus-option choice with a recommendation). Use for "write a proposal", "propose a fix for X", "draft a change to the spec for Y", after a real defect/metric/run surfaces a pattern worth fixing but not yet decided, or adding a capability to a tool with a SPEC.md. Pairs with spec-review/spec-build (usual evidence sources) and spec-writing (turns a confirmed proposal into real SPEC.md rows; a proposal never edits SPEC.md itself).
license: MIT
---
