---
name: spec-build
description: Implement a SPEC.md in any language or stack via test-driven development **executed wave by wave against an implementation plan this skill produces first**, then make README.md reflect the built reality, re-read SPEC.md, audit every produced artifact for conformance, and run and **look at** the product against the spec's reference images (or report VERIFICATION PENDING), writing SPEC_BUILD_REPORT.md. Phase 0 runs spec-plan to write IMPLEMENTATION_PLAN.md and one DETAILED_IMPLEMENTATION_PLAN_W<n>.md per wave; each later wave is implemented test-first, gated with its own commands, and committed before the next begins. Use when asked to "build the spec", "implement SPEC.md", "make this spec real", or after spec-writing/spec-review/spec-plan produce a Level-2/3 spec and plan. Pairs with spec-plan (the order this skill executes), spec-writing (authoring) and spec-review (auditing, yielding SPEC_REVIEW_REPORT.md + F-nnn findings and a P0/P1/P2 remediation plan).
license: MIT
---
...
| `WEAKLY_PASSING` | the judge found the test *executes* the behavior but never asserts on it | strengthen the test so it asserts the specified outcome; the judge only ever downgrades, never upgrades, so a mock-clean run can still be red here. One exception: a T-nn whose proof is a recorded run (self-application, evaluation, benchmark, observed pass) is cited by a presence check that *cannot* honestly assert it — that row belongs in the spec as `\| **T-nn** *(recorded)* \|` (spec-writing §9; speccheck SPEC v1.10+), which keeps the edge out of the judge; do not invent an assertion to satisfy the model |
