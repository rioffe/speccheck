...
retries, no batching (K-06):
  ...
  Body (JSON, exactly these keys):
    { "model": <SPECCHECK_JEV_MODEL>,
      "state": <TEXT, built from the C-06 request object for this edge -- {id, statement, declared,
...
| C-17 | `jev.py` (the four C-17 variables, the request body and `state` template, the `answers.verdict` parse and `p(e)`), `cli.py` (variable validation and redaction) | T-89, T-90 |
...
| K-16 | `jev.py` (triage pass, confidence ordering, D-30 Note), `judge.py` (issue order), `cli.py` (`--jev-pre-triage`) | T-89, T-90 |
...
Three ids this document does not declare are deliberately held rather than missing: `D-28`, `E-57` and `R-38` are claimed by the still-pending `PROPOSAL_v1.14_obligation_aware_judge.md` (its §5, §2 and §2 respectively), which has not been applied, so v1.15's own rows take D-29..D-32 and E-58..E-59 around them. Every other gap in a family is an error; these three are reservations, and the proposal that owns them will declare them when it lands.
