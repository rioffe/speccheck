...
> - **Status:** v1.11 — v1.10 with all seven findings of the v1.10 `spec-review` applied (`SPEC_REVIEW_REPORT.md`, F-401..F-407): `schema_version` is stated once, in C-07, and every §9 row refers to it (F-401); the C-06 validation list is ordered and first-match, a non-string `clause` is absent, a coerced `UNKNOWN` records `clause` `""` (F-402); the null-`verdict` condition names RECORDED ids in E-37, C-07 and T-34 (F-403); the `*(recorded)*` marker is one rule shared by both declaration forms and (b)'s title strips it, `~~T-48~~ (recorded)` pinned (F-404); `judge_strength` excludes recorded ids, as a formula (F-405); K-15 trims the clause (F-406); editorial (F-407). v1.10 settled D-22 (recorded tests, R-35); v1.9 added clause-grounded verdicts (R-34) and a body in the judge-evaluation fixture (T-76). Earlier versions: see the revision history.
