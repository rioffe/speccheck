...
```json
{
  ...
  "proof": {"build": {"exit": 0, "theorems_total": 66, "theorems_checked": 66, "errors": []}},  // v1.19, D-44/D-49; key omitted entirely unless --proof was given; "build" null unless --proof-results was also given
  "ids": [
    {
      "id": "R-07",
      "family": "R",
...
Proof field (v1.19, R-100, C-20, C-21, D-49): `proof` is an array of `{name, file, line, state,
  error}`, one entry per declaration that cites the id (shaped like `tests`); `state` is
  `checked`, `failed`, `unknown`, or `stale` (K-17); `error` is the build's error line for
  `failed`, else `null`. An id with `--proof` given but no citation has `proof: []`; the key is
  omitted entirely — on the top-level object and on every id — when `--proof` was not given. The
  top-level `proof.build` is C-21's `build` object echoed verbatim when `--proof-results` was
  also given, else `null`.
...
| D-49 | Exact JSON shape of `proof` (v1.19) | the confirming proposal pins conditional presence (D-44) and says the per-id field is "shaped like `tests`" but not its literal keys; this fold pins: per-id `proof` is an array of `{name, file, line, state, error}` (`state` $\in$ `{checked, failed, unknown, stale}`; `error` non-null only when `failed`; an id with `--proof` given but no citation has `proof: []`); the top-level `proof` is `{"build": {…} | null}`, echoing C-21's `build` verbatim when `--proof-results` was also given, else `null` | a fourth literal `"absent"` instead of an empty array — rejected: every other evidence array in this schema (`src`, `tests`) uses emptiness for "no citation", and a fifth token here would be the only exception; no top-level `proof` object at all, aggregate build info left to a walk of `ids` — rejected: `judge`/`judge_available` already set the precedent that aggregate run configuration belongs at the top | C-07, C-21, K-17 | requester / confirmed v1.19 (2026-09-24) |
