...
> - **Status:** v1.19.1 — one gap `spec-model`'s formal model of v1.19 found in v1.19 itself, fixed
>   before any implementation existed (F-504, `docs/reviews/SPEC_MODEL_FINDINGS.md`; D-49 amended,
>   2026-09-24). C-07's top-level `proof` key was gated on `--proof` alone; a run given only
>   `--proof-results` therefore read the manifest (C-21 states the read is unconditional) and wrote
>   no trace of it anywhere — the build result silently dropped. The gate is now `--proof` **or**
>   `--proof-results`; the per-id `proof` field is unaffected (still gated on `--proof` alone —
