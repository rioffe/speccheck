...
> - **Status:** v1.20 — three places where the spec's own formal model (`proof_from_spec/`, via
>   `spec-model`) could not render a total function over the input space it enumerates, each fixed
>   without waiting for a third-party review to find it first (`docs/proposals/PROPOSAL_v1.20_spec_determinacy.md`,
>   2026-09-24; D-46..D-48 confirmed on the recommended branches, same day). **(a)** `judge_available`
>   (C-07, amended) is now boolean and total: `false` whenever at least one edge was judge-eligible
>   and no judge call succeeded — whether every call failed (E-14) or none was issued at all, the
>   `--judge-budget`/`--jev-pre-triage` case E-35 already names (K-12) — and `true` otherwise; it is
>   never `null` under `mock`/`llm`. E-32 (amended) records `strict_judge_failure: "unavailable"` for
>   this case too, replacing a `null` reason on an exit-`1` run R-28's own reason pair exists to
>   prevent (T-101; D-46). **(b)** `explain --spec` (§5.1) is now stated as required in its synopsis,
>   not just its flag table — the two disagreed on one enumerated invocation, `explain ID` with no
>   `--spec`, and the model's `outcome` returned `none` for it; a missing `--spec` on `explain` is now
>   E-64, exit `2`, message `explain: --spec is required` (T-102; D-47). **(c)** §5.4 gains K-18: when
>   one invocation carries both a usage fault (exit `2`) and an input-contract violation (exit `3`),
>   flag/value/`--root`-containment validation completes before any spec/results/output read, so the
>   usage fault is reported — closing the one case where two faithful implementations could disagree
>   on the same invocation's exit code (T-103; D-48). No status, verdict, citation, or existing
>   metric moves; the one behavioural change is Part (a)'s exit reason on an already-red
>   `--strict --judge llm` run whose eligible edges were all budget-skipped (`null` → `"unavailable"`),
>   and the goldens are unaffected since `--self-check`/§9.8 run `--judge mock` with no budget.
>
> - **Previous status:** v1.19.1 — one gap `spec-model`'s formal model of v1.19 found in v1.19
>   itself, fixed before any implementation existed (F-504, `docs/reviews/SPEC_MODEL_FINDINGS.md`;
>   D-49 amended, 2026-09-24). C-07's top-level `proof` key was gated on `--proof` alone; a run given
>   only `--proof-results` therefore read the manifest (C-21 states the read is unconditional) and
>   wrote no trace of it anywhere — the build result silently dropped. The gate is now `--proof`
>   **or** `--proof-results`; the per-id `proof` field is unaffected (still gated on `--proof` alone
>   — there is nothing to attribute without a scan). No id, flag, exit code, or byte of a run under
>   either flag alone or both together changes; only the *no-`--proof-results`-alone* case gains a
>   `proof` key it previously lacked. Building the feature also refined K-17's file match: a real
>   cross-repository run against `hello_world_deepseek`'s own Lean proof showed every citation
>   reading `stale` instead of `checked`, because a citation's `file` is `--root`-relative (R-20)
>   while `speccheck/tools/proof_evidence.py`'s manifest records `file` relative to its own
>   `--proof` scan root — the join now tries exact match, then a path-suffix match, before
>   concluding the two name different declarations. v1.19 — the formal-verification half of the conformance graph gets a machine tie into the checker (R-100, C-20, C-21, I-018, K-17, E-62, E-63, E-65; T-99, T-100; `docs/proposals/PROPOSAL_v1.19_proof_parameter.md`, 2026-09-23; D-43..D-45 confirmed on the recommended branches, 2026-09-24). A repeatable, `--root`-contained `--proof PATHS` flag (C-20) scans Lean source files/directories, splits each on a declaration head (`theorem`/`lemma`/`def`/`abbrev`/`instance`), reads the spec ids from the doc comment directly above (bold spans, `(a)`-style sub-case markers stripped), and attributes a *proof citation* per (declaration, id) pair to a new IdRecord field, `proof` — shaped like `tests` (name, file, line) and never sent to the judge, since a kernel-checked declaration is a fact, not an assertion (R-100). `--proof-results FILE` (C-21) reads — never runs — a JSON manifest (`build`, `theorems[]`, optional `deferred_to_pytest[]`) and joins it to the citations by declaration name (+ file/line, K-17), giving each cited id a proof state — `checked`, `failed` (with the build's error line), `unknown` (cited, absent from the manifest; E-63), or `stale` (a manifest entry naming a different file/line) — a parallel field, never a status input: C-05's algorithm, every status, and `impact`'s blast radius are unchanged (R-06, E-63). `speccheck.json` gains a per-id `proof` field and a top-level `proof` object only when `--proof` is given, `schema_version` unchanged (D-44, D-49); an absent-flag run is byte-identical to v1.18 (I-018, T-100). Under `--strict`, a `failed` proof state is a gate failure only when `--proof-results` was given and is readable (D-45). Both flags are `check`-only in this version — `explain`/`impact` reject them (E-54's pattern), and `explain`'s C-18 mechanism is untouched. v1.18 — the CLI documents its own parameters and environment (R-41, C-19, I-017; T-95..T-98; `docs/proposals/PROPOSAL_v1.18_cli_help_contract.md`, 2026-09-20; D-37..D-42 confirmed on the recommended branches). Every argument definition of the three parsers carries a help entry naming purpose, accepted values as literal tokens, the default and every precondition (C-19), and every `--help` screen carries an `environment:` block naming the eight `SPECCHECK_*` variables with their read condition, requiredness and default plus `COLUMNS` (D-41), and an exit-code/summary-line epilog (D-40). The help's value tokens are the validator's own usage-error tokens (T-95), every definition must carry help and a C-19 metavar (T-96), the screens are byte-pinned at `COLUMNS=80` (T-97, D-39), and the environment block is bound to the code's own `SPECCHECK_*` literals (T-98). No accepted value, default or exit code changes; C-06's wrong variable name (`SPECCHECK_LLMMODEL` → `SPECCHECK_JUDGE_MODEL`) is fixed here because v1.16 and v1.17 had already landed (D-42). v1.17 — `speccheck explain <ID>` (R-40, C-18, I-016, E-60, E-61; `docs/proposals/PROPOSAL_v1.17_explain_id.md`, 2026-09-20; D-33..D-36 confirmed) adds a third subcommand that renders one id's evidence trail — its statement, its status and the C-05 step that set it, its source and test citations with each citing case's outcome and verdict, and the C-12 blast radius at `--depth` — as one stdout trace. It computes nothing new: it is a renderer over the `IdRecord`/edge/`Verdict` facts `check` already produces and the `walk`/`reverify` facts `impact` already runs (I-016), writes no report file (D-33), and carries `check`'s judge contract unchanged (E-61).
>
> - **Previous status:** v1.16 — a judged edge now receives `related`, the titles of the other obligations its own statement names and that name it (a C-12 `depends_on` neighbourhood, both directions), so the judge can tell an assertion about this clause apart from an assertion about a neighbour it refers to (`docs/proposals/PROPOSAL_v1.14_obligation_aware_judge.md`, 2026-09-20; D-28 and its sub-decision D-28b confirmed). R-38 requires the request to carry `related` — the neighbourhood, own references first, at most eight, each a whitespace-collapsed 160-character title, a retired neighbour tagged `(retired)` (E-57), `[]` when the id has no neighbour — and C-10 defines that an assertion corresponding only to a related obligation is not evidence this statement holds, advisory, no new coercion rule (D-28 extends D-26's stance); the field is request-side only, so no `speccheck.json` field, status, Markdown row, or `schema_version` changes, and it also rides the C-17 Jev triage `state` (D-28b), roughly doubling that input. v1.15 — a truncated judge budget can now be spent on the edges Jev is least sure about (`docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md`, 2026-09-20; D-29..D-32 confirmed). A new `--jev-pre-triage` flag (K-16) sends one triage task per judge-eligible edge — the C-06 request object for that edge, rendered as C-17's `state` text — to the C-17 Jev provider before any real-judge request is issued, and orders the otherwise unchanged real-judge issue loop (K-05, K-06, C-06) by ascending Jev top-choice probability, ties by ascending id; Jev's answer is read for that order and discarded (I-015). `--judge-budget` grows an `N%` form (K-12): exactly $\lceil N/100 \times E \rceil$ of the $E$ eligible edges, least-confident first, are issued, and the rest take today's `judge: budget` disposition; `N%` requires `--jev-pre-triage` under `--judge llm` (E-58), the `SECONDS` form is unchanged, and `0%` is a pure triage dry run. A per-edge Jev failure orders that edge first and is counted in one Note (E-59). No `speccheck.json` field, no status, no Markdown row, and no `schema_version` changes (D-29..D-32). v1.14 — a citation's own words now tell the checker, and the judge, whether a test *declares* an id or merely *reuses* it (`docs/proposals/PROPOSAL_v1.15_declared_vs_incidental_citations.md`, 2026-09-20; D-26 and D-27 confirmed). A citation of an in-scope R/C/I/K/E id inside an attributed test case is **DECLARED** when its line sits inside that case's own docstring or is a whole-line comment, and **INCIDENTAL** otherwise (C-03, C-14; R-39); `Citation.declared` and a new `JudgeRequest.declared` (C-06, C-15) carry the fact into the judge, and `C-10`'s instruction text asks the model not to credit `ASSERTS` on an INCIDENTAL citation without checking the assertion is actually about that clause — advisory only, no new coercion rule (D-26's `yes` branch). `speccheck.json` gains `tests[].declared` and `metrics.declared_ratio` under every `--judge` mode, including `none` (C-07, C-16, `schema_version` `"1.5"`). Detection is whole-line-comment only, reusing the doc-comment-line recognition the Swift adapter already has rather than adding token-level parsing (D-27's `yes` branch; E-56, I-014). Statuses, the mock judge, and the Markdown report are unchanged. v1.13 made the edges this document already carries into data and added a second subcommand that walks them (D-24, D-25); v1.12 made `--src`/`--tests` comma-separated lists of files and/or directories (D-23, I-012, E-52); v1.11 applied F-401..F-407; v1.10 settled D-22 (R-35); v1.9 added clause-grounded verdicts (R-34). Earlier versions: see the revision history.
...
> - **Sources:** `one_sentence_prompt.md` (the brief); `../skills/spec-writing/SKILL.md` (the ID taxonomy and `SPEC.md` shape the checker consumes); `../skills/spec-build/SKILL.md` §Phase 3 (the manual conformance audit this tool automates); `../skills/spec-review/SKILL.md` §3.17 (the intent → requirement → contract → invariant → test → evidence chain); `../outline.md` Chapters 15–18 (where this system is the worked example); `docs/reviews/SPEC_REVIEW_REPORT.md` (one file, rewritten per review: v0.1 → F-001..F-017, v1.3 → F-201..F-210, v1.7 → F-301..F-307, v1.10 → F-401..F-407; all cited below); `FINAL_SPEC_REVIEW_REPORT.md` (review of v0.4; F-101..F-110 below point at it); `SPEC_v0.5_REVIEW_REPORT_by_QWEN.md` (independent review of v0.5 by a second model; its F-001..F-011 are cited below as Q-001..Q-011 to avoid collision); `docs/proposals/PROPOSAL_v1.7_heading_bodies.md` (the v1.7 change, its evidence, and D-20); `docs/proposals/PROPOSAL_v1.9_clause_grounding.md` (the v1.9 change, its two-model evidence, D-21, and the D-22 question); `docs/proposals/PROPOSAL_v1.13_impact.md` (the v1.13 change, its measurement of the edges in this document — 186 `depends_on`, 240 `verifies`, 110 `affects` in v1.12 — and D-24, D-25); `docs/proposals/PROPOSAL_v1.15_declared_vs_incidental_citations.md` (the v1.14 change; `JUDGE_CROSSCHECK_REPORT.md` §2b's cross-model evidence — 153/613 genuine judge conflicts on this document's own `check --judge llm` run, concentrated in ids reused as generic fixture data — and D-26, D-27); `docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md` (the v1.15 change; its §1 calibration measurement — Jev's top-choice probability bucketed against a real `check --judge llm --strict` run's committed verdicts on this repository's own `SPEC.md`, monotonic from 53/111 = 47.75% below 0.60 to 279/295 = 94.58% at $\geq$ 0.95 over 582 edges, and 51.5% of all judge-eligible edges already at $\geq$ 0.95 — plus D-29..D-32); `docs/proposals/PROPOSAL_v1.14_obligation_aware_judge.md` (the v1.16 change; its sec. 1 evidence — the cross-checker's finding that a test can cite an id as placeholder data while asserting a fact about a neighbour it names, `JUDGE_CROSSCHECK_REPORT.md` sec. 2b — plus D-28 and its D-28b sub-decision)
...
```json
{
  ...
  "proof": {"build": {"exit": 0, "theorems_total": 66, "theorems_checked": 66, "errors": []}},  // v1.19.1, D-44/D-49 (amended, F-504); key omitted entirely unless --proof or --proof-results was given; "build" null unless --proof-results was given
  "ids": [
    {
      "id": "R-07",
      "family": "R",
      ...
      "proof": [ {"name": "rowInterrupt", "file": "proof/Theorems.lean", "line": 105, "state": "checked", "error": null} ],  // v1.19; [] when --proof was given but this id has no citation; key omitted entirely without --proof (D-49)
      "tests": [
        {
...
Title and statement (R-33): `title` is `SpecId.title` and `statement` is `SpecId.text` (C-01, C-02).
  ...
  `schema_version` does NOT change in v1.19: the top-level `proof` object and each id's `proof`
  field (R-100, C-20, C-21) are conditional keys, present only when `--proof` was given (D-44,
  D-49) — unlike every prior addition above, which was unconditional and so bumped the version.
...
Proof field (v1.19, amended v1.19.1 per F-504; R-100, C-20, C-21, D-49): `proof` is an array of
  `{name, file, line, state, error}`, one entry per declaration that cites the id (shaped like
  `tests`); `state` is `checked`, `failed`, `unknown`, or `stale` (K-17); `error` is the build's
  error line for `failed`, else `null`. An id with `--proof` given but no citation has `proof: []`;
  the **per-id** key is omitted entirely when `--proof` was not given (there is nothing to
  attribute without a scan). The **top-level** `proof` key is present whenever `--proof` OR
  `--proof-results` was given (v1.19.1: was `--proof` alone, which silently dropped a manifest
  read via `--proof-results` on its own, `docs/reviews/SPEC_MODEL_FINDINGS.md` F-504); its
  `proof.build` is C-21's `build` object echoed verbatim when `--proof-results` was given, else
  `null`.
...
Metrics:
  ...
  proof_failed       = |{id : any proof[].state == "failed"}|                (v1.19, K-17; present only when --proof given)
  proof_unknown      = |{id : no "failed" proof[], any proof[].state in {"unknown","stale"}}|   (v1.19, K-17; present only when --proof given)
...
Shape rules (F-016): `by_family` ALWAYS has exactly the six keys R, C, I, K, E, T in that order,
  ...
  `judge_available` is `null` under `--judge none`. With `--judge mock` or `--judge llm` it is
  `true` when at least one judge call succeeded, or when no edge was judge-eligible (I-010;
  vacuously available), and `false` when at least one edge was judge-eligible and no judge call
  succeeded — whether because every call failed (E-14) or because no call was issued at all
  (E-35: a budget of `0`, or the `SECONDS`/`N%` deadline reached before the first request). It is
  never `null` under `mock`/`llm` (v1.20, amended; F-502). The C-07 example's `// comments` are
  explanatory and never emitted.
...
# v1.19: when --proof was given, the row gains a sixth cell, `Proof (state)`, rendering each C-07
#   proof[] entry as `name (file:line, state)`, semicolon-joined, or an em dash when proof[] is
#   empty; the column is absent entirely without the flag (I-018)
...
SPECCHECK_JEV_MODEL    optional, default ~typesafe/jev-latest  (OpenRouter's floating alias -- not
                       the pinned typesafe/jev-1.13 the calibration in docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md
  ...
  Body (JSON, exactly these keys):
    { "model": <SPECCHECK_JEV_MODEL>,
      "state": <TEXT, built from the C-06 request object for this edge -- {id, statement, related, declared,
                ...
                figures of docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md §1 were measured on>,
...
### C-20 The lean adapter (v1.19; R-100)

`lean.py`, beside `swift.py`, splits a Lean source on declaration heads — lines matching
`theorem`, `lemma`, `def`, `abbrev` or `instance` (each optionally `public`/`protected`-prefixed),
one declaration per head — and, for each declaration, reads the spec ids from the doc-comment
block ending within one line above the head: every `[RCIKE]-nn` id inside a bold span (`**C-02**`,
`**R-11, I-003**`; a mixed list `**E-08, D-17, R-03**` contributes its R/C/I/K/E members only),
with `(a)`-style parentheticals stripped. A directory named by `--proof` is scanned recursively
for `*.lean`. A file whose declarations carry no tag contributes no citation; a file that is
unreadable, not Lean source, or yields no parseable declaration head contributes a `notes` entry,
not an error (E-62). The adapter never reads a declaration body — it is a citation scanner over
doc comments, exactly as `swift.py` is a citation scanner over test-case delimiters, not a
semantic check on what the declaration proves (§0 non-goal).

### C-21 The proof manifest (v1.19; R-100)

```json
{
  "build": {"exit": 0, "theorems_total": 66, "theorems_checked": 66, "errors": []},
  "theorems": [
    {"file": "HelloProof/Hello/Theorems.lean", "name": "rowInterrupt", "line": 105,
     "ids": ["C-04", "E-03", "R-09"], "status": "checked"}
  ],
  "deferred_to_pytest": [
    {"id": "R-09", "scope": "process side", "content": "real SIGINT delivery, plain 130 status",
     "tests": ["T-20"]}
  ]
}
```

speccheck reads exactly these three top-level keys and ignores every other one the file carries —
a producer MAY write provenance keys (`spec`, `source`, `proof_repo`, `pytest`) for a human
reader; they are not part of this contract, and neither is any extra key on a `theorems`/
`deferred_to_pytest` entry (`speccheck/tools/proof_evidence.py` writes both today). `build.errors`
is a list of `"<file>:<line>"` strings. A manifest theorem joins a proof citation (C-20) on name
and file (line, where present; K-17): a citation without a matching manifest entry is `unknown`,
a manifest entry without a citation is a `notes` entry. `--proof-results FILE` given but
unreadable, not well-formed JSON, or lacking `build`/`theorems` → E-65.
...
| `--spec FILE` | Required on `check` and `explain` alike (v1.20; on `explain`, a missing flag is E-64's own message). Path to the specification; decoded as UTF-8 with `errors="replace"` (a Note is recorded if any byte was replaced). Missing flag or unreadable file → usage error. | `2` |
...
| `--proof PATHS` | `check`-only (v1.19, R-100). Repeatable; each value a comma-separated PATHS list of Lean source files/directories, scanned per C-20 and resolved inside `--root` (E-09). Absent → no Lean file scanned, no `proof` key written (I-018). | `2` |
| `--proof-results FILE` | `check`-only (v1.19, C-21). A JSON proof manifest, read and joined to the C-20 citations (K-17); the checker never runs a prover. Absent → every citation's state is `unknown` (no manifest to confirm it). Unreadable, malformed, or missing `build`/`theorems` → E-65. | `2` / `3` |
...
**Precedence between `2` and `3` (v1.20, K-18).** Flag, value and `--root`-containment validation (the usage-fault class, `2`) is completed in full before `SPEC.md`, the results file, the output directory, or (for `impact`) `--against` is read at all — so an invocation carrying both a usage fault and what would otherwise be an input-contract violation (`3`) exits `2`; a run never reaches the contract check that would have failed. No other precedence exists between the two classes, since no other invocation can trigger both.
...
| **I-018** | **Proof-input inertness (v1.19).** A `check` run without `--proof`/`--proof-results` writes `speccheck.json` and `SPEC_CONFORMANCE_REPORT.md` byte-identical to a v1.18 run on the same inputs: no `proof` key in the JSON (top-level or per-id), no proof column in the Markdown, no Lean file read, no exit-code difference (T-100). |
...
| **K-17** | **Proof staleness (v1.19; the file match refined v1.19.1).** A manifest theorem (C-21) whose name matches a scanned declaration (C-20) in a *different* file or line is `stale`, surfaced like a stale citation; a scanned declaration absent from the manifest is `unknown`, never `failed`; a `failed` state carries the build error line from `build.errors`. The file comparison is exact-match first, then a path-suffix match (either side a suffix of the other): a citation's `file` is posix-relative to `--root` (R-20), while a manifest a foreign producer wrote commonly records `file` relative to its own `--proof` scan root instead — `proof/HelloProof/Hello/Theorems.lean` (citation) and `HelloProof/Hello/Theorems.lean` (manifest) name the same file and MUST NOT join as `stale` on that account alone. Stale/unknown/failed proof states appear in `notes` and the `proof_failed`/`proof_unknown` metrics and, per D-45, affect `--strict`. |
| **K-18** | **Exit-code precedence (v1.20).** When one invocation carries both a usage fault (§5.4 `2`: an undefined flag, an out-of-range or malformed value, a missing required flag, a path outside `--root`) and an input-contract violation (§5.4 `3`: E-01, E-02, E-03, E-05, E-18), the usage fault is reported: flag, value and `--root`-containment validation are completed before `SPEC.md`, the results file, the output directory or `--against` is read at all, so a run never reaches the contract check that would have failed. No other precedence exists — the two are never both live. |
...
| **E-09** | Any of `--spec/--src/--tests/--results/--out/--proof/--proof-results` (v1.19) — including any element of a `--src`/`--tests`/`--proof` PATHS list — resolves outside `--root` (after symlink resolution) | Exit `2`, message `path outside --root: <path>`. A PATHS element that is itself a symlink and resolves inside `--root` is used in resolved form and descended (a directory) or scanned (a file); it is not an E-30 skip (Q-007, D-23). |
...
| **E-32** | `--strict --judge llm` and either `judge_available == false` or `unknown_rate > max_unknown` | Exit `1` even if every status is `PASSING`; `strict_judge_failure` set to `"unavailable"` when `judge_available == false` — whether the judge failed (E-14) or the eligible edges were never issued (E-35) — else `"unknown_rate"` when `unknown_rate > max_unknown`; when both hold, `"unavailable"` (F-012, Q-004; v1.20, amended). Summary line carries the matching suffix. Without `--strict`, or with `--judge none|mock`, no effect. |
...
| **E-54** | `impact` given both `--changed` and `--against`, or neither; or given `--results`, any judge flag, or `--proof`/`--proof-results` (v1.19) | Exit `2`, message `impact: exactly one of --changed, --against is required` (or `impact: --results is not accepted` / `impact: --judge is not accepted` / `impact: --proof is not accepted`); no reports written. An `--against` file that is unreadable or outside `--root` is the ordinary usage error (E-09); one that fails C-01 is exit `3` with `--against: ` prefixed to the E-01/E-02/E-03 message. |
...
| **E-62** | An unreadable or non-Lean file in a `--proof` path (binary, or no declaration head parseable) | Recorded as a `notes` entry naming the file; MUST NOT be a usage error and MUST NOT change the exit code of a run whose ids are all otherwise decided (v1.19). |
| **E-63** | A proof state (`checked`/`failed`/`unknown`/`stale`) is computed for an id | It never changes that id's status: C-05 reads citations, tests, results and the judge only — a `checked` proof never promotes an `UNVERIFIED` id, and a `failed`/`unknown`/`stale` proof never demotes a `PASSING` one (R-06). `impact` MUST ignore proof citations: a theorem's file is not a scanned source file, so proof carries no blast radius (v1.19). |
| **E-64** | `explain` invoked without `--spec` (v1.20) | The flag is required on this subparser (D-47), and its synopsis carries no brackets (E-54's pattern). Exit `2`, message `explain: --spec is required`, and no trace is written; no `explain` stage runs. |
| **E-65** | `--proof-results FILE` given but unreadable, not well-formed JSON, or lacking the `build`/`theorems` keys C-21 requires | Unreadable → exit `2`, message `--proof-results: <reason>`; malformed JSON or missing required keys → exit `3`, message `proof-results: <reason>` (the `--results`/E-05 pattern; v1.19). No report is written. |
...
| **T-103** | A run that carries both fault kinds — a `--src` element that is neither file nor directory (E-52) together with a malformed `--results` (E-05) — exits `2`, not `3`, and writes no report; the same two faults split across separate invocations exit `2` and `3` respectively, so the precedence is observable and not merely asserted. (K-18, §5.4; v1.20) |
...
| ID | Test |
| -- | ---- |
| **T-99** | Over the §9.8 golden fixture extended with a minimal Lean proof (three declarations, two tagged, one untagged) and a two-entry manifest: `check --spec … --src … --tests … --results … --proof <fixture lean> --proof-results <fixture manifest>` exits as the golden pins; the JSON carries, for the tagged declarations, `proof` citations with state `checked`, for the untagged declaration no citation, for the manifest-failed entry state `failed` with the error line, for the cited-but-unmanifested entry `unknown`; the untagged file with no parseable declaration head yields a `notes` entry, not an error (E-62); the Markdown shows the proof column; `grep -c "proof"` on both goldens is the pinned count; a `--proof-results` file that is unreadable, not well-formed JSON, or missing `build`/`theorems` exits `2`/`3` per E-65; and a run over the `hello_world_deepseek` sidecar's evidence (`proof` + `build/proof/proof-results.json`, both produced by `speccheck/tools/proof_evidence.py`, under `--root .`) exits `0` with every dual id showing state `checked` — a cross-repository integration recorded, not pooled (R-24); `impact --spec … --proof proof` and `explain R-01 --spec … --proof-results build/proof/proof-results.json` each exit `2` with the amended E-54 message, naming `--proof`. (R-100, C-20, C-21, K-17, E-62, E-65, E-54) |
| **T-100** | Byte-identity under flag absence: `check` over the unextended §9.8 fixture, with no `--proof`/`--proof-results`, reproduces both v1.18 goldens byte for byte (I-018), and the rendered `check --help` (T-97's golden) shows exactly two new flag rows, each with C-19-compliant help. (I-018, C-19) |
...
- **Kernel dependencies:** none beyond the standard library (`re`, `ast`, `xml.etree.ElementTree`, `json`, `argparse`, `pathlib`, `logging`, `hashlib`). The lean adapter (`lean.py`, v1.19, C-20) is the same line-based, standard-library-only pattern as `swift.py` — no Lean toolchain is required to run `check` itself, since `--proof-results` is read, never executed (C-21).
...
- **Golden fixture layout:** `fixtures/target/{SPEC.md, src/, tests/, junit.xml, golden/speccheck.json, golden/SPEC_CONFORMANCE_REPORT.md, golden/impact.json, golden/IMPACT_REPORT.md, proof/, proof-results.json}` (`impact.json`/`IMPACT_REPORT.md` since v1.13, T-80; `proof/` — a three-declaration Lean fixture — and `proof-results.json` — its two-entry manifest — since v1.19, T-99); `golden/` is never a scan root and is never written to by a run (Q-003).
...
- **Tools (not collected by pytest, not part of the wheel):** `tools/bench.py` (T-51), `tools/eval_judge.py` (T-49), `tools/sync_selfcheck.py` (T-60), `tools/impact_backtest.py` (T-82; the only one that needs `git` and the repository history; D-24), `tools/proof_evidence.py` (v1.19; a standalone sidecar — reads a project's own `proof/` lake project and test suite, writes the C-21 manifest `--proof-results` reads; not part of the kernel and not required to run `check`, T-99's cross-repository integration case).
- **Commands:**
...
uv run speccheck check --spec SPEC.md --src src --tests tests --results junit.xml --proof proof --proof-results build/proof/proof-results.json --judge mock   # v1.19
```
...
| R-41 | `cli.py` (the help strings, metavars and epilogs of the four parsers) | T-95, T-96, T-97, T-98, T-102 |
| R-100 | `lean.py` (new adapter), `cli.py` (`--proof` parsing) | T-99 |
...
| C-07 | `report.py` (`to_json`; `title`; `recorded`; `clause`; `decisions`; `edges`; `declared`; `declared_ratio`; Decimal quantization; `verdict: null`; Note order; `judge_available` total over the eligible-but-unissued case, v1.20) | T-34, T-37, T-59, T-73, T-75, T-77, T-79, T-86, T-88, T-101 |
...
| C-19 | `cli.py` (every argument definition's help, the C-19 metavar vocabulary, the `environment:` and exit-code epilogs) | T-95, T-96, T-97, T-98, T-102 |
| C-20 | `lean.py` (declaration split, doc-comment tag extraction) | T-99 |
| C-21 | `lean.py`/`cli.py` (manifest read and join) | T-99 |
...
| I-018 | `cli.py`, `report.py` (conditional `proof` key, absent-flag byte-identity) | T-100 |
...
| K-17 | `lean.py` (join on name/file/line; stale/unknown/failed states) | T-99 |
| K-18 | `cli.py` (validation stage order: flags/values/containment before spec/results/out reads) | T-103 |
...
| E-09 | `cli.py` (path validation incl. `--out`; a PATHS element resolved and used, incl. neither-file-nor-directory → E-52) | T-40, T-78, T-99 |
...
| E-32 | `cli.py` (R-28 gate, `unavailable` precedence, extended to E-35 v1.20) | T-59, T-101 |
...
| E-54 | `cli.py` (`impact` flag exclusivity; rejected `check`-only flags; `--against` errors) | T-81, T-99 |
...
| E-64 | `cli.py` (`explain`'s required `--spec`, its message) | T-102 |
| E-61 | `cli.py` (the judge flags carried over), `explain.py` (verdict rendering incl. `coerced` and `not judged`) | T-94 |
| E-62 | `lean.py` (unreadable/non-Lean file → notes entry) | T-99 |
| E-63 | `graph.py` (C-05 untouched by proof), `impact.py` (proof edges ignored) | T-99 |
| E-65 | `cli.py` (`--proof-results` read/validation) | T-99 |
...
| D-24 | Where the change-impact backtest lives | `tools/impact_backtest.py`: a script outside the kernel that shells out to `git` for two spec versions and a build range, calls `speccheck impact --against`, and scores the prediction against the touched citations (T-82). The kernel exposes only the pure function of files (`--against FILE`) and never invokes `git` or any subprocess. | a kernel `--since REF` that reads the prior spec from `git` (one command for the operator, but the kernel gains a subprocess, a history-dependent input, and a failure mode per git state; I-001/I-002's "same bytes in, same bytes out" would need a git-state clause) | R-37, C-13, §3.1, §10, T-82, I-002 | requester / confirmed 2026-09-19 (`docs/proposals/PROPOSAL_v1.13_impact.md` §5) |
| D-25 | How decision rows enter the JSON | a separate `decisions` array (C-07) under a declaration-only family D (C-01 (c)): never in `ids`, never a citation target, never in `by_status`, `by_family`, `conformance`, `dangling`, or `stale`; I-003 and every existing golden §3 row are untouched | a seventh family in `ids` with a status of its own (uniform with the other families, but every denominator, the T-46 goldens, and the C-08 table change, `by_family` needs a seventh key, and `UNCITED`/`UNTESTED` are meaningless for a decision) | R-36, C-01, C-02, C-07, C-12, I-003, T-79 | requester / confirmed 2026-09-19 (`docs/proposals/PROPOSAL_v1.13_impact.md` §5) |
| D-26 | Whether `declared: false` changes the judge's verdict on its own | advisory only (Part B as written): `C-10`'s instruction text tells the model `declared` and asks it not to credit `ASSERTS` on an INCIDENTAL citation without checking the assertion's subject, but no C-06 rule coerces the verdict — preserves the model's judgment for the real, undocumented-but-correct cases; T-87 measures whether the text alone is enough | hard coercion: `declared: false` forces `ASSERTS` to `EXECUTES_ONLY` automatically, no model discretion — stronger and fully deterministic, but risks false downgrades on tests that simply predate or don't follow the docstring convention perfectly; kept as the fallback if T-87 shows the advisory text moves nothing | C-06, C-10, C-15, T-87 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.15_declared_vs_incidental_citations.md` §5) |
| D-27 | How a citation line is classified DECLARED | whole-line-comment detection: the first non-whitespace character on the citation's own line is `#` (Python) or the line is a doc-comment line by the Swift adapter's existing recognition (C-14) — no new parsing machinery, matches every real DECLARED example found while drafting the proposal | full token-level comment detection (via `tokenize`, distinguishing a trailing comment from code): more precise (would credit `x = 1  # R-02` as DECLARED) but needs column-accurate token classification the citation scanner does not carry today, for a case that did not appear once in the real evidence read for the proposal; left for a later proposal if it proves necessary | C-03, C-14, T-85 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.15_declared_vs_incidental_citations.md` §5) |
| D-28 | How wide the judge request's `related` neighbourhood reaches, and whether it rides the Jev triage `state` (this row carries both: the reach decision and its sub-decision, recorded here as **D-28b**) | **(a)** `related` is the titles of the obligations this statement names **and** of those that name it (C-12 `depends_on`, both directions), the statement's own references first, at most eight total, each a whitespace-collapsed title tail-truncated to 160 characters with `…`, `[]` when the id has no neighbour; a retired neighbour keeps its title with `(retired)` appended (E-57). `(b)` **D-28b**: `related` is included in the C-17 triage `state` (not stripped) — Jev ranks the exact request the real judge gets — at a cost of roughly doubling the per-edge triage input, which is cents (§3 of `docs/proposals/PROPOSAL_v1.14_obligation_aware_judge.md`). | **(a-alt)** own references only, uncapped: smaller on heavily-depended-on ids (C-01 and C-07 have dozens of dependents) but misses the reverse-direction case the §4 flags — a test that cites E-09 yet asserts E-52's message is the same off-topic `ASSERTS` the other way round. **(b-alt)** strip `related` from the triage `state`: cheaper (no neighbourhood pass) at the price of a weaker proxy for "least-sure-about"; kept as the fallback if a T-89 run shows triage ranking is insensitive to the neighbourhood. Advisory: the `related` rule in C-10 tells the model not to credit an `ASSERTS` that matches only a related obligation, but no C-06 rule coerces a verdict for it. | R-38, C-06, C-10, C-16, C-17, K-16, I-015, E-57, T-83, T-84, T-89 | requester / confirmed 2026-09-20 (reach — both directions, own-first, cap 8 — per `docs/proposals/PROPOSAL_v1.14_obligation_aware_judge.md` §5; the D-28b sub-decision — include in the triage `state` — per that proposal's §7 addendum; both the recommended options) |
| D-29 | How the triage provider is configured | dedicated variables, `SPECCHECK_JEV_URL` / `SPECCHECK_JEV_MODEL` / `SPECCHECK_JEV_API_KEY` / `SPECCHECK_JEV_TIMEOUT` (C-17), read only when the triage pass runs | reusing `OPENROUTER_API_KEY`, as `tools/jev_client.py` already does — one fewer variable to set, but it couples the kernel's own configuration to a `tools/`-script naming choice and cannot be scoped or rotated independently of any other OpenRouter use | C-17, K-16, §10 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md` §5) |
| D-30 | Whether `--jev-pre-triage` with `--judge-budget 0` complains | yes — one informational Note, `jev-pre-triage had no effect: --judge-budget is unlimited` (K-16). The combination pays Jev's latency and dollar cost for zero effect on the report, and a Note costs nothing while catching a likely mistake, consistent with this project's Notes-not-errors style | silently proceeding — no new Note text, but the one combination that buys nothing looks exactly like the one that buys something | K-16, K-12, E-59, T-89 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md` §5) |
| D-31 | What a per-edge Jev failure during triage does | orders that edge first (as least confident) and records one Note (E-59); the run is never aborted. This session's own `deepseek-v4.1-flash` self-check run hit 110/656 (16.8%) real-provider coercions — 65 timeouts, 42 malformed responses — so "Jev could not answer" is expected to happen, and treating it as "assume this edge needs the real judge" is the safe default for a signal that sometimes fails | aborting the whole run on the first Jev failure — simpler and louder, but fragile against exactly the provider flakiness this project has already measured in a sibling judge model; it would also let a triage-only fault destroy a run that would otherwise have completed | E-59, K-16, T-89 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md` §5) |
| D-32 | How `N%` rounds | up: $\lceil N/100 \times E \rceil$ of the $E$ eligible edges (K-12), so any nonzero percentage issues at least one edge to the real judge | floor or round-to-nearest — floor silently zeroes the real judge at low percentages on a small edge count (1% of 50 edges floors to 0, a pure triage run that does not say so) | K-12, T-89 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.16_jev_pre_triage.md` §5) |
| D-33 | `explain`'s output surface (v1.17) | **stdout only**: no `--out`, no durable file, no `schema_version` entry, no golden pair — the structured view of the same facts already exists as `speccheck.json`, and the narrative is what this command adds (C-18, §3.3) | a durable `EXPLAIN_<ID>.md` + `explain.json` pair matching the two-files-per-subcommand pattern — CI-friendlier, but it adds a durable-artifact contract, a golden pair to regress, and a third pair that must obey §3.1's temp-and-rename rule, while duplicating `speccheck.json` | R-40, C-18, §3.3, I-001, T-92 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.17_explain_id.md` §5, recommended branch) |
| D-34 | Whether the trace carries the upward `impact` section (v1.17) | **include** it, at `--depth 1` by default: the forensic complement to the downward trace — *why this id is weak* beside *what a change to it might touch* — and it costs nothing, since C-12's `walk` and C-13's `reverify` already exist (C-18) | downward-only (the pure "why" trace), which would leave the reader to run `impact` separately for the other half | R-40, C-18, C-13, T-92 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.17_explain_id.md` §5, recommended branch) |
| D-35 | Where `explain`'s facts come from (v1.17) | **recompute** from the same pipeline inputs `check` uses (`--spec`/`--src`/`--tests`/`--results`, judge contract carried over, E-61): deterministic, byte-reproducible, able to show a live verdict under `--judge llm`, and no second input contract | a `--from speccheck.json` fast-path — instant, but it adds a report-as-input surface §0's boundary cautions against and cannot show a verdict a `none`/`mock` run never recorded | R-40, C-18, I-016, §0, T-92 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.17_explain_id.md` §5, recommended branch) |
| D-36 | How many ids one `explain` invocation takes (v1.17) | **one id per invocation**, positional; an undeclared id is exit `2` per E-60 and a retired id is rendered, not an error; a shell loop drives many ids | accepting a list of ids and printing one block each — convenient, but it would need a second output format and its own exit-code rule (does one undeclared id fail the whole run?) | R-40, C-18, E-60, K-01, T-93 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.17_explain_id.md` §5, recommended branch) |
| D-37 | How much a flag's help entry carries (v1.18) | **the full entry** — purpose, values, default and preconditions (C-19) | values and default only (shorter, but it leaves E-58's `N%` companion requirement and E-09's containment rule undocumented — the two failures §1 shows rejecting a well-formed command); values only (the literal ask, and the help still cannot be used to construct a judge invocation) | R-41, C-19, T-95, T-96 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.18_cli_help_contract.md` §5, recommended branch) |
| D-38 | Whether the exact help wording is normative (v1.18) | **content-required, wording free** — C-19 pins what each entry must carry and the metavar vocabulary; T-97's golden in `tests/data/help/` makes every wording change visible without a spec revision | exact strings normative in `SPEC.md` (maximum stability for consumers that parse the help, at a spec revision and version bump per typo, and ~50 lines of CLI prose duplicating §5.1) | R-41, C-19, T-97, §10 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.18_cli_help_contract.md` §5, recommended branch) |
| D-39 | How the help is guarded (v1.18) | **round-trip equality plus width-pinned goldens** — T-95 (the tokens are true), T-96 (every definition has help), T-97 (the bytes, at `COLUMNS=80`) | round-trip equality only (no golden churn on rewording, but a silently reduced entry then passes as long as its remaining tokens are correct) | R-41, C-19, T-95, T-96, T-97 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.18_cli_help_contract.md` §5, recommended branch) |
| D-40 | What the screens carry beyond the per-flag entries (v1.18) | **per-flag help plus an exit-code and summary-line epilog** on every `--help` (the codes are the second thing an operator or agent needs, and they are printed nowhere the CLI can be asked about; §5.4 is a stable K-01-pinned set) | strictly per-flag help (smallest surface, no fourth copy of §5.4); per-flag help plus a machine-readable `--help-json` (exact for a parsing agent, but a new interface, a new contract, and a second rendering of the same facts to keep equal) | R-41, C-19, §5.4, §5.1, T-96, T-97 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.18_cli_help_contract.md` §5, recommended branch) |
| D-41 | What the `environment:` block lists (v1.18) | **the eight `SPECCHECK_*` names with read condition, requiredness and default, plus one clause for `COLUMNS`** — the only variable that changes a `--help` run's own output, and the one T-97's golden pins | the eight names only (`COLUMNS` stays undocumented and T-97's fixed width unexplained); also documenting `install.sh`'s `~/.config/speccheck/judge.env` and its shell-rc `source` line (helpful for a first-time installer, but no kernel code reads that file — it is the installer's shell convenience) | R-41, C-19, T-97, T-98 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.18_cli_help_contract.md` §5, recommended branch) |
| D-42 | Where C-06's wrong variable name is fixed (v1.18) | **in this version's own fold** — `SPECCHECK_LLMMODEL` becomes `SPECCHECK_JUDGE_MODEL`, matching C-06's own request block two lines above it, C-09's block and `judge_llm.py`'s `ENV_MODEL`; the proposal's recommended branch (ride along in the still-uncommitted v1.16 fold) is no longer available because v1.16 and v1.17 have landed, so the fix lands here | leaving it (the name stays wrong and contradicts C-06's own block; T-98 still passes, because it binds the help to the code, not to that sentence) | R-41, C-19, C-06, C-09, T-98 | requester / confirmed 2026-09-20 (`docs/proposals/PROPOSAL_v1.18_cli_help_contract.md` §5; branch adapted — see the v1.18 history row) |
| D-43 | Proof manifest granularity (v1.19) | **per-declaration**: name + line + `checked`/`failed`, one entry per declaration (C-21) — it is the granularity the build tool emits and the per-id table needs; a single failed declaration still names itself | per-file check status — cheaper manifest, but a one-declaration failure fails every id the file tags, and T-99's pinned per-declaration states lose their referent | C-21, K-17, T-99 | requester / confirmed v1.19 (`docs/proposals/PROPOSAL_v1.19_proof_parameter.md` §5, recommended branch, 2026-09-24) |
| D-44 | Whether `speccheck.json`'s `proof` key is conditional (v1.19) | **conditional**: the top-level `proof` object and the per-id `proof` field exist only when `--proof`/`--proof-results` were given; `schema_version` unchanged; absent-flag output byte-identical to v1.18 (I-018, T-100) — zero golden churn and zero consumer impact for the existing user base; a missing key unambiguously means "no proof input" | always-present `proof: null` — a stable key set, but every existing golden and every downstream consumer of the current shape must change, and `schema_version` must bump | C-07, I-018, T-100 | requester / confirmed v1.19 (`docs/proposals/PROPOSAL_v1.19_proof_parameter.md` §5, recommended branch, 2026-09-24) |
| D-45 | Whether a `failed` proof trips `--strict` (v1.19) | **only when `--proof-results` was given and is readable**: the gate fails on a broken proof it was explicitly handed, while a missing manifest keeps the gate exactly as robust as v1.18 — a repo that never had a proof is never punished for the absence of one, and a proof failure is a defect of the proof repo, not the checked system | proof states strictly advisory in all modes — maximally decoupled, but "did the formal half break?" never answers through `--strict` | K-17, E-63, §5.4 | requester / confirmed v1.19 (`docs/proposals/PROPOSAL_v1.19_proof_parameter.md` §5, recommended branch, 2026-09-24) |
| D-49 | Exact JSON shape of `proof` (v1.19, amended v1.19.1) | the confirming proposal pins conditional presence (D-44) and says the per-id field is "shaped like `tests`" but not its literal keys; this fold pins: per-id `proof` is an array of `{name, file, line, state, error}` (`state` $\in$ `{checked, failed, unknown, stale}`; `error` non-null only when `failed`; an id with `--proof` given but no citation has `proof: []`; key omitted per-id only when `--proof` was not given); the top-level `proof` is `{"build": {…} | null}`, present whenever `--proof` **or** `--proof-results` was given (amended v1.19.1: `proof_from_spec`'s modeling of this row found a run given only `--proof-results` read the manifest and wrote no `proof` key at all under the original `--proof`-only gate — F-504, `docs/reviews/SPEC_MODEL_FINDINGS.md`), echoing C-21's `build` verbatim when `--proof-results` was given, else `null` | a fourth literal `"absent"` instead of an empty array — rejected: every other evidence array in this schema (`src`, `tests`) uses emptiness for "no citation", and a fifth token here would be the only exception; no top-level `proof` object at all, aggregate build info left to a walk of `ids` — rejected: `judge`/`judge_available` already set the precedent that aggregate run configuration belongs at the top; (v1.19.1) leaving the `--proof`-only gate and documenting `--proof-results`-alone as an intentional no-op — rejected: a flag silently producing no observable effect at all is worse than widening its gate by one clause | C-07, C-21, K-17 | requester / confirmed v1.19 (2026-09-24); amended v1.19.1 (2026-09-24, F-504) |
| D-46 | Whether `judge_available` becomes boolean and total over the eligible-but-unissued case (v1.20) | **boolean, total**: `false` whenever at least one edge was judge-eligible and no call succeeded, whether every call failed (E-14) or none was issued (E-35) — the field is what R-28 gates on, and "no usable judge" is one operational fact however it arose; T-101 measures both budget forms | leave C-07's two clauses and patch only E-32's reason — smaller diff, but `speccheck.json` keeps a field the contract doesn't fully determine for this cell | C-07, E-32, E-35, R-28, T-101 | requester / confirmed v1.20 (`docs/proposals/PROPOSAL_v1.20_spec_determinacy.md` §5, recommended branch, 2026-09-24) |
| D-47 | Whether `--spec` becomes genuinely required on `explain` (v1.20) | **required**, synopsis brackets dropped — states existing behaviour (the flag table already said `Required.`; there was already no defined way to run `explain` without a spec) rather than inventing a new one; T-102 pins it | make `--spec` optional with a stated default — smaller for a quick `explain R-01`, but invents a fallback file the spec has never had and adds a second place the input spec is chosen | R-40, C-18, C-19, E-64, T-102 | requester / confirmed v1.20 (`docs/proposals/PROPOSAL_v1.20_spec_determinacy.md` §5, recommended branch, 2026-09-24) |
| D-48 | Whether exit-code precedence (usage fault vs. input-contract violation) is pinned as a normative `K` row (v1.20) | **pinned**, K-18, beside K-01's exit set — one sentence, checkable by T-103 | leave §5.4 silent — two faithful implementations could then exit `2` or `3` on the same invocation | K-01, K-18, §5.4, T-103 | requester / confirmed v1.20 (`docs/proposals/PROPOSAL_v1.20_spec_determinacy.md` §5, recommended branch, 2026-09-24) |
...
D-28 (and its sub-decision D-28b), E-57 and R-38 — the obligation-aware judge of `docs/proposals/PROPOSAL_v1.14_obligation_aware_judge.md` — are now declared by this v1.16 fold, discharging the reservation that held them in v1.15; D-29..D-32 and E-58..E-59 sit in the v1.15 Jev rows. The reservation that stood for the still-pending `docs/proposals/PROPOSAL_v1.17_explain_id.md` in v1.16 is discharged: v1.17 declares R-40, C-18, I-016, E-60, E-61 and T-92..T-94 for it. D-43..D-45 — the proof-parameter feature of `docs/proposals/PROPOSAL_v1.19_proof_parameter.md` — are now declared by this v1.19 fold, discharging the reservation that held R-100, C-20, C-21, I-018, K-17, E-62, E-63, T-99, T-100; D-49 — the JSON shape left unpinned by that proposal — was confirmed separately, 2026-09-24, then amended the same day in the v1.19.1 fold once `proof_from_spec/`'s model of it found F-504. D-46..D-48 — the three determinacy fixes of `docs/proposals/PROPOSAL_v1.20_spec_determinacy.md` — are now declared by this v1.20 fold, discharging the reservation that held K-18, E-64, T-101..T-103. `docs/proposals/PROPOSAL_obligation_census.md` remains pending and reserves no ids. Every gap in a family not held by such a pending proposal is an error, and the proposal that owns a held id will declare it when it lands.
